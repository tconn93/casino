...
class CrapsGame {
  ...
  evaluateBet(bet, roll) {
    ...
    // Come and Don't Come bets are handled separately in evaluateBets()
    ...
    // Any 7
    if (bet.type === 'any7' && total === 7) {
      return { win: true, payout: 5 };
    }
    ...
    // Specific numbers
    if (bet.type === 'craps2' && total === 2) {
      return { win: true, payout: 31 };
    }
    if (bet.type === 'craps3' && total === 3) {
      return { win: true, payout: 16 };
    }
    if (bet.type === 'yo11' && total === 11) {
      return { win: true, payout: 16 };
    }
    if (bet.type === 'craps12' && total === 12) {
      return { win: true, payout: 31 };
    }
    ...
    if (bet.type.startsWith('place')) {
      ...
      if (total === number) {
        if (number === 4 || number === 10) return { win: true, payout: 2.8 }; // 9:5
        if (number === 5 || number === 9) return { win: true, payout: 2.4 }; // 7:5
        if (number === 6 || number === 8) return { win: true, payout: 2.167 }; // 7:6
      }
      if (total === 7) return { win: false, payout: 0 };
    }

    // Hardways
    if (bet.type.startsWith('hard') && roll.die1 === roll.die2) {
      const number = parseInt(bet.type.replace('hard', ''));
      if (roll.die1 + roll.die2 === number) {
        if (number === 4 || number === 10) return { win: true, payout: 8 }; // 7:1
        if (number === 6 || number === 8) return { win: true, payout: 10 }; // 9:1
      }
    }
    if (bet.type.startsWith('hard') && (total === 7 || roll.die1 !== roll.die2)) {
      return { win: false, payout: 0 };
    }
...
async function handleAction(table, socket, data) {
  ...
  if (action === 'placeBet') {
    ...
    // Prevent Come/Don't Come bets during come out phase
    if ((betType === 'come' || betType === 'dontCome') && table.gameState.phase === 'comeOut') {
      throw new Error('Come and Don\'t Come bets can only be placed after the point is established');
    }
  ...
  if (action === 'roll') {
    ...
    for (const player of table.players) {
      if (results.has(player.socketId)) {
        const winAmount = results.get(player.socketId);
        const totalBet = game.bets.get(player.socketId)
          ? game.bets.get(player.socketId).reduce((sum, bet) => sum + bet.amount, 0)
          : 0;

        if (winAmount > 0) {
          await Wallet.credit(player.userId, winAmount, 'craps', 'Craps win');
          await User.updateStats(player.userId, 'craps', true, totalBet, winAmount);
        } else if (totalBet > 0) {
          await User.updateStats(player.userId, 'craps', false, totalBet, 0);
        }
    ...
    if (game.phase === 'comeOut') {
      if (![2, 3, 7, 11, 12].includes(total)) {
        ...
        // Reset bets for new come-out roll
        game.bets.clear();
      }
    } else {
